...
Reading DF
"""
df = pd.read_csv(r'drone_merged_2021-05-TM 2021-10-TM 2022-03-TM_maxdist100_angletol15.csv')
df['DateTime'] = pd.to_datetime(df['date'] + ' ' + df['time'])
df = df[df['Distance'] < 100]
df = df[df['Gimbal Degree'].between(80, 100)]
...
for k in range(len(attributes_wqp)):
    ...
    thresh = corr.loc[wqp, rrs_columns + index_columns].quantile(0.75)
    corr_secchi = corr_secchi[abs(corr_secchi)>=thresh].dropna(how='all', axis=0)
...
for z in range(len(dis_list)):
    ...
    results.to_csv('./resultados_modelagem/' + df_folder[z]+'/' + csv_filenames[z] + '_merged' + '.csv')
    df_all = pd.concat([df_all, results], axis = 0)
df_all.to_csv('./resultados_modelagem/all_wqp_results_merged.csv')
